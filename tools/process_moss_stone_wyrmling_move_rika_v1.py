...
MAGENTA_SHEET_PATH = VERSION_ROOT / "source/moss_stone_wyrmling_move_sheet_chroma_v1.png"
SKILL_OUTPUT_DIR = VERSION_ROOT / "processor_qc"
FRAME_DIR = VERSION_ROOT / "frames"
SHEET_PATH = ANIMATION_ROOT / "moss_stone_wyrmling_move_sheet_v1.png"
OVERVIEW_PATH = VERSION_ROOT / "moss_stone_wyrmling_move_overview_v1.png"
ZOOM_OVERVIEW_PATH = ROOT / "output/moss_stone_wyrmling_move_frames_4x_v1.png"
GIF_PATH = ROOT / "output/moss_stone_wyrmling_move_preview_v1.gif"
...
def write_verified_gif(
    ...
    exact = len(decoded_frames) == len(previews) and all(
        ImageChops.difference(decoded_frame, preview).getbbox() is None
        for decoded_frame, preview in zip(decoded_frames, previews, strict=True)
    )
    expected_durations = [duration_ms] * len(previews)
    passed = exact and durations == expected_durations and all(
        method == 2 for method in disposal_methods
    )
    if not passed:
...
def main() -> None:
    ...
    magenta = make_magenta_sheet(raw)
    save_image(magenta, MAGENTA_SHEET_PATH)
    run_skill_processor()
    ...
    preview_overview = Image.new("RGBA", transparent_sheet.size, (*PREVIEW_BACKGROUND, 255))
    preview_overview.alpha_composite(transparent_sheet)
    save_image(
        preview_overview.resize(
            (preview_overview.width * 4, preview_overview.height * 4),
