...
IMAGE_ROOT = ROOT / "output/sprite_animation_candidates/azure_wind_feather/move_v1"
SOURCE_DIR = IMAGE_ROOT / "source"
RAW_DIR = IMAGE_ROOT / "raw"
MANIFEST_DIR = ROOT / "output/sprite_animation_candidates/_manifests/azure_wind_feather/move_v1"
SOURCE_PATH = SOURCE_DIR / "azure_wind_feather_source_v1.png"
TRANSPARENT_INPUT_PATH = SOURCE_DIR / "azure_wind_feather_transparent_128_v1.png"
RIKA_INPUT_PATH = SOURCE_DIR / "azure_wind_feather_rika_start_128_v1.png"
PROMPT_PATH = SOURCE_DIR / "prompt-used.txt"
INPUT_PREVIEW_PATH = ROOT / "output/azure_wind_feather_rika_input_4x_v1.png"
...
PROMPT = (
    "right-facing three-quarter side-view cute four-legged celestial feather "
    "creature performs one slow in-place walking cycle that reads as walking "
    "across exactly one grid cell, front-left and rear-right legs step together "
    "then front-right and rear-left legs step together, clear alternating "
    "quadruped footfalls, gentle shoulder and hip weight transfer, small natural "
    "follow-through in the large cyan feathered ears, back plumes and tail plumes, "
    "body center remains fixed and all feet return to one stable baseline, no "
    "running, no hopping, no root translation, no turning, no camera movement, "
    "preserve the exact fluffy white body silhouette, bright cyan and deep blue "
    "crystal-feather appendages, black hard pixel outline, single visible cyan eye, "
    "gold forehead and cheek markings, golden paws, palette, proportions, face and "
    "pixel-art material, no extra legs, no extra wings, no detached effects, no "
    "background motion"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare and optionally submit one Azure Wind Feather Rika move job."
    )
    parser.add_argument(
