...
RIKA_INPUT_PATH = SOURCE_DIR / "aqua_crystal_frog_rika_input_128_v1.png"
INPUT_PREVIEW_PATH = ROOT / "output/aqua_crystal_frog_rika_input_4x_v1.png"
...
PROMPT = (
    "front-facing aqua crystal frog performs one readable in-place jump cycle: "
    "crouch and compress, push off with both hind legs, rise into the air, extend "
    "the hind legs backward, descend, land on all four feet, then settle into the "
    "exact starting pose; keep the body centered horizontally and keep the start "
    "and end feet on the same baseline; preserve the exact huge navy eyes, mint "
    "belly, turquoise skin, blue crystal back spikes, spots, silhouette, palette, "
    "pixel clusters and proportions; no turning, no camera movement, no extra "
    "limbs, no projectile, no dust, no detached effects, no background motion"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare and optionally submit one aqua-frog Rika jump job."
    )
    parser.add_argument(
...
def get_api_key() -> str:
    # A long-running editor/Codex process can retain an older inherited value
    # after the user updates the Windows user environment. Prefer the live
    # HKCU value on Windows so a key rotation takes effect without a restart.
    if os.name == "nt":
        import winreg
...
def prepare_input() -> None:
    ...
    if alpha.getextrema() != (0, 255):
        raise RuntimeError("Expected a hard-edged transparent pixel-art source")

    # The wide frog needs extra headroom for a readable jump. Scale the complete
    # source canvas as one unit to preserve all edge pixels and identity markers.
    target_width = 96
    ...
    if bbox is None:
        raise RuntimeError("Prepared input is empty")
    print(f"PREPARED_INPUT={RIKA_INPUT_PATH}")
    print(f"SUBJECT_BBOX={bbox}")
