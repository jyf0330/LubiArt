...
IMAGE_ROOT = ROOT / "output/sprite_animation_candidates/frost_spirit_wolf/move_v1"
SOURCE_DIR = IMAGE_ROOT / "source"
RAW_DIR = IMAGE_ROOT / "raw"
MANIFEST_DIR = ROOT / "output/sprite_animation_candidates/_manifests/frost_spirit_wolf/move_v1"
SOURCE_PATH = SOURCE_DIR / "frost_spirit_wolf_source_v1.png"
CLEAN_SOURCE_PATH = SOURCE_DIR / "frost_spirit_wolf_clean_v1.png"
TRANSPARENT_INPUT_PATH = SOURCE_DIR / "frost_spirit_wolf_transparent_128_v1.png"
RIKA_INPUT_PATH = SOURCE_DIR / "frost_spirit_wolf_rika_start_128_v1.png"
PROMPT_PATH = SOURCE_DIR / "prompt-used.txt"
INPUT_PREVIEW_PATH = ROOT / "output/frost_spirit_wolf_rika_input_4x_v1.png"
...
PROMPT = (
    "left-facing three-quarter side-view cute four-legged frost spirit wolf "
    "performs one slow in-place walking cycle that reads as walking across exactly "
    "one grid cell, front-left and rear-right legs step together then front-right "
    "and rear-left legs step together, clear alternating quadruped footfalls, gentle "
    "shoulder and hip weight transfer, subtle head settling, small delayed follow-through "
    "in the large flame-shaped ice ears, icy back crest and enormous curled frost tail, "
    "body center remains fixed and all feet return to one stable baseline, no running, "
    "no hopping, no root translation, no turning, no camera movement, preserve the exact "
    "compact wolf silhouette, lavender-blue body, pale fluffy chest and muzzle, cyan glowing "
    "eye, cyan neck flame, huge blue-white curled tail, black hard pixel outline, palette, "
    "proportions, face and pixel-art material, no extra legs, no missing legs, no extra tails, "
    "no detached effects, no ice projectile, no background motion"
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare and optionally submit one Frost Spirit Wolf Rika move job."
    )
    parser.add_argument(
