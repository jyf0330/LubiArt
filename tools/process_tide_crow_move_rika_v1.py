...
CHROMA_SHEET_PATH = VERSION_ROOT / "source/tide_crow_move_sheet_chroma_v1.png"
SKILL_OUTPUT_DIR = VERSION_ROOT / "processor_qc"
FRAME_DIR = VERSION_ROOT / "frames"
SHEET_PATH = ANIMATION_ROOT / "tide_crow_move_sheet_v1.png"
OVERVIEW_PATH = VERSION_ROOT / "tide_crow_move_overview_v1.png"
ZOOM_OVERVIEW_PATH = ROOT / "output/tide_crow_move_frames_4x_v1.png"
...
def make_chroma_sheet(raw: Image.Image) -> Image.Image:
    output = raw.convert("RGB")
    pixels = output.load()
    for y in range(output.height):
        for x in range(output.width):
            if color_distance(pixels[x, y], BACKGROUND_KEY) <= CHROMA_DISTANCE:
...
def extract_frames(raw: Image.Image) -> list[Image.Image]:
    expected_size = (
    ...
    for index in range(FRAME_COUNT):
        ...
        for y in range(FRAME_SIZE[1]):
            for x in range(FRAME_SIZE[0]):
                ...
                alpha = (
                    0
                    if color_distance(color, BACKGROUND_KEY) <= CHROMA_DISTANCE
                    else 255
                )
        ...
        mask = (rgba_array[:, :, 3] > 0).astype(np.uint8)
        count, labels, stats, _ = cv2.connectedComponentsWithStats(mask, 8)
        if count <= 1:
            raise ValueError(f"Frame {index + 1} has no foreground component")
        largest_label = 1 + int(np.argmax(stats[1:, cv2.CC_STAT_AREA]))
        rgba_array[:, :, 3] = np.where(labels == largest_label, 255, 0).astype(
            np.uint8
        )
...
def load_processed_frames() -> list[Image.Image]:
    frames: list[Image.Image] = []
    for index in range(1, FRAME_COUNT + 1):
        path = SKILL_OUTPUT_DIR / f"frame-{index}.png"
        if not path.exists():
            raise FileNotFoundError(f"Processed frame is missing: {path}")
        frame = Image.open(path).convert("RGBA")
        if frame.size != FRAME_SIZE:
            raise ValueError(f"Unexpected processed frame size: {path}: {frame.size}")
        frames.append(frame)
    return frames
...
def write_verified_gif(
    ...
    indexed_frames = build_exact_palette(previews)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def main() -> None:
    ...
    save_image(make_chroma_sheet(raw), CHROMA_SHEET_PATH)
    run_skill_processor()
    ...
    slow_gif_durations = write_verified_gif(
    ...
    skill_source_edge_touch_frames = skill_meta.get("source_edge_touch_frames") or []
    has_source_edge_touch = bool(
        source_edge_touch_frames or skill_source_edge_touch_frames
    )
    blocking_issues = []
    if has_source_edge_touch:
        blocking_issues.append(
            "Source silhouette touches a source-cell edge; missing pixels cannot be restored by local processing."
        )
    report = {
        "version": 1,
        "service": "Rika AI",
        "generation_id": submission["gen_id"],
        "credits_before": job.get("credits_before", submission.get("credits_before")),
        "credits_after": job.get("credits_after"),
        "actual_credit_change": job.get("actual_credit_change"),
        ...
        "skill_processor_qc": {
            ...
            "source_edge_touch_frames": skill_source_edge_touch_frames,
        ...
        "technical_qa_status": "BLOCKED" if has_source_edge_touch else "PASS",
        "visual_qa_status": "BLOCKED" if has_source_edge_touch else "PENDING_MANUAL_REVIEW",
    }
    ...
    print("SLOW_GIF_EXACT_PIXEL_MATCH=True")
