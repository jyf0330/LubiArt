...
- `CellDetail` 直接挂 `battle_cell_detail.gd`，只管点选格子后显示的宠物 / 地形信息。交付文件 `宠物信息栏实装(修改).psd` 是其中宠物格子信息的美术源；476×539 卡片与动态数据接口在 `sprite_info_card.tscn` 中维护，`pet_detail.tscn` 负责遮罩、操作与调试容器，项目运行时不依赖 PSD 或其本机路径。
...
- 修改战斗整屏、HUD 或特效层级时直接改本 Scene 或 `core_ui/scripts/battle/`；修改宠物详情卡面时打开 `sprite_info_card.tscn`，修改遮罩、操作或调试面板时打开 `pet_detail.tscn`。
