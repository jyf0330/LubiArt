...
func plugin_id() -> String: return "hp_percent_below"
func matches(condition: Dictionary, context: Dictionary) -> bool: return _hp(context) < int(condition.get("value", 0))
func _hp(context: Dictionary) -> int:
	...
	return int(max(0, int(unit.get("hp", 0))) * 1000 / maximum)
