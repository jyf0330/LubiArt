...
func after_hit(port: RefCounted, target: Dictionary, source: Dictionary, _amount: int, _damage_props: Dictionary, mechanism: Dictionary) -> Array:
	...
	if bool(flags.get("reflectUsed", false)):
		return []
	flags["reflectUsed"] = true
	target["flags"] = flags
	var reflect := int(port.mechanic_param_int(target, mechanism, "reflect", 2))
