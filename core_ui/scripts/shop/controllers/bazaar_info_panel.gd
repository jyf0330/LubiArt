...
func render_snapshot(snap: Dictionary, view: StringName) -> void:
	...
	visible = view == &"shop" or view == &"bag" or view == &"reward" or view == &"battle_end" or view == &"day_end" or view == &"game_over"
...
func _bag_page_count(snap: Dictionary) -> int:
	return max(1, int(ceil(float(_bench_count(snap)) / float(BAG_PAGE_SIZE))))
...
func _with_feedback(base_text: String) -> String:
	if _feedback_text == "":
		return base_text
	if _feedback_pending:
		return "%s\n\n%s" % [base_text, RuntimeUiPolicy.text("UI_RESULT_PENDING", [_feedback_text])]
