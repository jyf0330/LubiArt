...
var _last_command_response_mode := ""
...
func _render_route(snap: Dictionary) -> void:
	...
	for index in range(_three_buttons.size()):
		...
		if has_option:
			...
			var portrait := _shop_character_texture(option) if kind == "shop" else null
			if portrait == null:
				portrait = _route_slot_texture(index)
			var kind_icon := _route_icon_texture(kind) if kind == "battle" else _route_slot_icon_texture(index)
			_render_route_card(slot, button, portrait, kind_icon, index)
			button.set_meta("command", Dictionary(card.get("command", {})))
			button.set_meta("route_kind", kind)
...
func _on_three_pressed(index: int) -> void:
	...
	if not await _submit_core_command(command):
		return
	var target_view := _render_after_core_command(command, _take_core_command_snapshot())
	await _transition_to_view(target_view)
...
func _on_shop_button_down(index: int) -> void:
	...
	if item_type != "宠物":
		...
		if await _submit_core_command(command):
			_render_after_core_command(command, _take_core_command_snapshot())
		return
	_close_pet_context_detail()
...
func _on_shop_back_pressed() -> void:
	...
	if not await _submit_core_command({"type": "EXIT_SHOP"}):
		return
	var target_view := _render_after_core_command({"type": "EXIT_SHOP"}, _take_core_command_snapshot())
	await _transition_to_view(target_view)
...
func _transition_to_view(target_view: StringName) -> void:
	if target_view == _current_view:
		_show_view(target_view)
		return
	if _current_view in [VIEW_THREE_OPTION, VIEW_SHOP] and target_view in [VIEW_THREE_OPTION, VIEW_SHOP]:
		# Route/shop are high-frequency decision surfaces. Their authority result is
		# already confirmed, so reveal the prepared target immediately and reserve
		# the longer cinematic transition for battle/reward boundaries.
		_show_view(target_view)
		return
	_is_transitioning = true
	_set_game_cursor_loading(&"view_transition", true)
...
func _ensure_legacy_code_shop_view() -> void:
	...
	_legacy_code_shop_view.call("configure", Callable(self, "_pet_texture"), Callable(self, "_shop_character_texture"))
...
func _render_bazaar_information(snap: Dictionary, target_view: StringName) -> void:
	...
	if _bazaar_info_panel != null and _bazaar_info_panel.has_method("render_snapshot"):
		# The info panel only needs one page-local scalar in addition to the
		# immutable authoritative Snapshot, so keep nested payloads shared.
		var panel_snapshot := snap.duplicate(false)
...
func _on_bazaar_info_command_requested(command: Dictionary) -> void:
	...
	if String(command.get("type", "")) == "UI_SET_BAG_PAGE":
		_apply_bag_page(int(command.get("page", 0)))
		return
	if not await _submit_core_command(command):
		return
	var target_view := _render_after_core_command(command, _take_core_command_snapshot())
...
func _apply_bag_page(page: int, snapshot_override: Dictionary = {}) -> void:
	_bag_page = max(0, page)
	var snap := snapshot_override.duplicate(true) if not snapshot_override.is_empty() else _current_snapshot()
	_render_roster(snap)
...
func toggle_quick_save_load_toolbar() -> bool:
	return _developer_toolbar.toggle_quick_access()


func is_quick_save_load_toolbar_visible() -> bool:
	return _developer_toolbar.is_quick_access_visible()
...
func _submit_core_command(command: Dictionary) -> bool:
	...
	_publish_command_pending(command, before_snapshot)
	# Small route/shop commands are expected to settle before the next paint, so
	# do not force an extra frame of pending UI in front of their confirmed result.
	# Complex and battle commands still yield once to make their loading state visible.
	var command_type := String(command.get("type", ""))
	if is_inside_tree() and command_type not in ["CHOOSE_ROUTE", "ENTER_SHOP", "BUY_OFFER", "ROLL_SHOP", "EXIT_SHOP", "BACK_TO_ROUTE"]:
		await get_tree().process_frame
	var response := Dictionary(await _request_broker.request_command(command))
	_set_game_cursor_loading(&"session_command", false)
	var after_snapshot := Dictionary(response.get("snapshot", before_snapshot))
	_snapshot = after_snapshot
	if bool(response.get("accepted", false)):
		_last_command_snapshot = after_snapshot
		_last_command_response_mode = String(response.get("snapshotMode", "full"))
		_publish_command_feedback(command, response, before_snapshot, after_snapshot, true)
		return true
	command_type = String(response.get("command", command.get("type", "")))
	var error := Dictionary(response.get("error", {}))
	var reason := String(error.get("message", error.get("code", "rejected")))
...
func _publish_command_pending(command: Dictionary, before: Dictionary) -> void:
	...
	match command_type:
		"BUY_OFFER":
			...
			message = RuntimeUiPolicy.text("UI_PENDING_BUY", [String(offer.get("name", offer.get("id", "商品")))])
		"ROLL_SHOP":
			message = RuntimeUiPolicy.text("UI_PENDING_REFRESH")
		"EXIT_SHOP":
			message = RuntimeUiPolicy.text("UI_PENDING_EXIT_SHOP")
		_:
			return
	_ensure_bazaar_info_panel()
	if _bazaar_info_panel != null and _bazaar_info_panel.has_method("show_command_pending"):
		_bazaar_info_panel.call("show_command_pending", message)
	_ensure_legacy_code_shop_view()
	if _legacy_code_shop_view != null and _legacy_code_shop_view.has_method("show_command_pending"):
		_legacy_code_shop_view.call("show_command_pending", RuntimeUiPolicy.text("UI_RESULT_PENDING", [message]))
...
func _render_after_core_command(command: Dictionary, snap: Dictionary) -> StringName:
	var response_mode := _last_command_response_mode
	_last_command_response_mode = ""
	if response_mode != "delta":
		return _render_content_from_state(snap)
	var command_type := String(command.get("type", ""))
	match command_type:
		"CHOOSE_ROUTE", "ENTER_SHOP", "BUY_OFFER", "ROLL_SHOP":
			_ensure_legacy_code_shop_view()
			if _legacy_code_shop_view != null and _legacy_code_shop_view.has_method("render_command_delta"):
				_legacy_code_shop_view.call("render_command_delta", snap, command_type)
			else:
				_render_legacy_code_shop(snap)
			return VIEW_SHOP
		"EXIT_SHOP", "BACK_TO_ROUTE":
			# Choosing a route shop advances the authoritative schedule before the
			# stall opens. Rebind the shared HUD, roster and hidden route cards from
			# the confirmed projection before revealing them so purchases and the next
			# node remain visible after the incremental shop view is closed.
			_render_hud(snap)
			_render_roster(snap)
			_render_route(snap)
			return VIEW_THREE_OPTION
...
func _shop_character_texture(record: Dictionary) -> Texture2D:
	return _asset_registry.shop_character_texture(record)
