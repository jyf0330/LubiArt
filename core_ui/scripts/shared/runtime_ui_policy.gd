...
const ZH_CN := {
	...
	"UI_RESULT_PENDING": "处理中：%s…",
	"UI_PENDING_BUY": "购买%s",
	"UI_PENDING_REFRESH": "刷新商店",
	"UI_PENDING_EXIT_SHOP": "离开商店",
	...
	"UI_ACTION_SELECT_PET": "4 宠物共享技能条已就绪；选宠可查看红色作用格",
	"UI_ACTION_PREVIEW": "预览：红色作用格 %d 个 · 共享条 %d 个 A/B 技能从左到右触发",
	"UI_ACTION_RESET_READY": "回溯（可用%d次）",
	"UI_ACTION_RESET_THRESHOLD": "回溯（需存活≤%d，可用%d次）",
	"UI_ACTION_RESET_WAIT": "回溯（第%d回合+1次，距下次%d回合）",
	"UI_ACTION_ALL_OUT": "释放 4 宠物全部 A/B 技能",
	...
	"UI_ACTION_QUEUE_HINT": "4 宠物共享技能控制条 · 拖拽或确认键排序",
...
const EN := {
	...
	"UI_RESULT_PENDING": "Processing: %s…",
	"UI_PENDING_BUY": "buy %s",
	"UI_PENDING_REFRESH": "refresh shop",
	"UI_PENDING_EXIT_SHOP": "leave shop",
	...
	"UI_ACTION_SELECT_PET": "The shared 4-pet skill bar is ready; select a pet to preview its target cells",
	"UI_ACTION_PREVIEW": "Preview: %d red target cells · %d shared A/B skills resolve left to right",
	"UI_ACTION_RESET_READY": "Rewind (%d available)",
	"UI_ACTION_RESET_THRESHOLD": "Rewind (alive ≤ %d, %d available)",
	"UI_ACTION_RESET_WAIT": "Rewind (+1 on round %d, next in %d rounds)",
	"UI_ACTION_ALL_OUT": "Trigger All A/B Skills for 4 Pets",
	...
	"UI_ACTION_QUEUE_HINT": "Shared 4-Pet Skill Bar · Drag or confirm to reorder",
	...
	"UI_POSITION_SKIPPED": "%s positioning: Not executed",
