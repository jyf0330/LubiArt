...
const OUTPUT_DIR := "res://output/visible_volcanic_dijiang_idle_battle"
...
func _run() -> void:
	...
	for sample in range(5):
		...
		if image.save_png(path) != OK:
			push_error("VISIBLE_VOLCANIC_DIJIANG_IDLE_BATTLE_FAIL could not save %s" % path)
			quit(1)
			return
		await create_timer(0.68).timeout
	print("VISIBLE_VOLCANIC_DIJIANG_IDLE_BATTLE_PASS output=%s" % output_dir)
	quit(0)
...
func _snapshot() -> Dictionary:
	var units := [
		{
			"id": "volcanic_dijiang_idle",
			"pet_id": "pal_042",
			"name": "Volcanic Dijiang",
			"side": "player",
			"hp": 10,
			"max_hp": 10,
			"shield": 0,
	...
	return {
		"phase": "battle",
		"stateVersion": 1,
		"stateHash": "volcanic-dijiang-idle-visible",
		"battle_round": 1,
