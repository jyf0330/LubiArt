...
const OUTPUT_DIR := "res://output/visible_azure_thunder_owl_idle_battle"
...
func _run() -> void:
	...
	for sample in range(5):
		...
		if image.save_png(path) != OK:
			push_error("VISIBLE_AZURE_THUNDER_OWL_IDLE_BATTLE_FAIL could not save %s" % path)
			quit(1)
			return
		await create_timer(1.2).timeout
	print("VISIBLE_AZURE_THUNDER_OWL_IDLE_BATTLE_PASS output=%s" % output_dir)
	quit(0)
...
func _snapshot() -> Dictionary:
	...
	return {
		"phase": "battle",
		"stateVersion": 1,
		"stateHash": "azure-thunder-owl-idle-visible",
		"battle_round": 1,
