...
const SOURCE_PATH := "res://art/images/shared/pets/animations/spr_057_frost_fox/anim_001_idle/frames/frame_001.png"
const OUTPUT_DIR := "res://output/qa/frost_fox/battle_runtime"
...
func _run() -> void:
	...
	if pet == null or target_cell == null:
		...
		return
	pet.call("play_grid_movement", pet.global_position, target_cell.global_position, MOVE_DURATION)
	for sample in range(9):
		if not await _capture("%s/move_%02d.png" % [OUTPUT_DIR, sample + 1]):
			return
		await create_timer(MOVE_DURATION / 8.0).timeout
	print("VISIBLE_FROST_FOX_BATTLE_PASS output=%s" % OUTPUT_DIR)
	quit(0)
...
func _snapshot() -> Dictionary:
	...
	return {
		"phase": "battle",
		"stateVersion": 1,
		"stateHash": "frost-fox-battle-visible",
		"battle_round": 1,
...
func _fail(message: String) -> void:
	push_error("VISIBLE_FROST_FOX_BATTLE_FAIL: %s" % message)
	quit(1)
