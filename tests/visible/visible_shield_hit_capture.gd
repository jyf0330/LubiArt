...
func _run() -> void:
	...
	var output_dir := ProjectSettings.globalize_path("res://output/shield_hit_godot_frames")
	...
	for frame_index in range(FRAME_COUNT):
		...
		if frame_index == 2:
			viewport_image.save_png(ProjectSettings.globalize_path("res://output/shield_hit_godot_preview_1920x1080.png"))
		var crop_rect := _centered_crop_rect(pet_center, viewport_image.get_size())
		var cropped := viewport_image.get_region(crop_rect)
		var frame_path := output_dir.path_join("shield_hit_godot_frame_%03d.png" % (frame_index + 1))
	...
	print("VISIBLE_SHIELD_HIT_PASS: %s" % output_dir)
	quit(0)


func _find_pet(battle_view: Control) -> Control:
	...
	for cell in board_grid.get_children():
		...
		if pet != null and pet.has_method("play_damage_feedback"):
			return pet
	return null
...
func _fail(message: String) -> void:
	push_error("VISIBLE_SHIELD_HIT_FAIL: %s" % message)
	quit(1)
