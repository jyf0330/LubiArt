...
const EXPECTED_BUTTON_RECTS := {
	...
	"AttackOrderButton": Rect2(1773.0, 915.0, 65.0, 62.0),
...
const EXPECTED_TOOLTIPS := {
	...
	"SpeedButton": "战斗日志：打开或关闭战斗日志看板。（快捷键 D）",
...
func _run() -> void:
	...
	var expected_gaps := [3.0, 3.0, 3.0]
	for index in range(1, ordered_buttons.size()):
		assert(ordered_buttons[index].position.y == ordered_buttons.front().position.y)
		assert(
			ordered_buttons[index].position.x - ordered_buttons[index - 1].get_rect().end.x
			== expected_gaps[index - 1]
		)
	...
	prefab.log_requested.connect(_on_log_requested)
