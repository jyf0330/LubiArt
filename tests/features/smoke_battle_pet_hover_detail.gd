...
func _assert_visible_detail(battle: Control, expected_name: String) -> bool:
	...
	if not (card.get_node_or_null("Margin/Column/Health") is UiValueBar):
		_fail("Battle detail must compose the shared value-bar component.")
		return false
	return true
...
func _assert_context_layout(battle: Control) -> bool:
	...
	if not panel.scale.is_equal_approx(Vector2.ONE):
		_fail("Battle-only compact detail must keep exact source scale, got %s." % panel.scale)
		return false
	...
	if rect.position.x < 0.0 or rect.position.x > 40.0 or rect.position.y < 0.0 or rect.position.y > 40.0:
		...
		return false
	if not rect.size.is_equal_approx(Vector2(380.0, 820.0)):
		_fail("Battle-only compact detail should preserve the authored 380x820 canvas, got rect %s." % rect)
		return false
	return true
